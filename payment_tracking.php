...
if(isset($_POST['record_payment'])){
    ...
    $or_num = !empty($_POST['or_number']) ? htmlspecialchars($_POST['or_number']) : 'OR-' . strtoupper(uniqid());
    
    // THIS TEXT IS CRUCIAL: It must match what statement_of_account is looking for
    $type_text = ($payment_type == 'DP') ? 'Down Payment' : 'Monthly Amortization';
    ...
    if($amt > 0){
        try {
            ...
            if($check_stmt->get_result()->num_rows > 0) {
                ...
                if ($has_res_id_col) {
                    $stmt = $conn->prepare("INSERT INTO transactions (reservation_id, type, amount, transaction_date, description, or_number, user_id) VALUES (?, 'INCOME', ?, ?, ?, ?, ?)");
                    $stmt->bind_param("idssss", $res_id, $amt, $date, $desc, $or_num, $admin_id);
                } else {
                    $stmt = $conn->prepare("INSERT INTO transactions (type, amount, transaction_date, description, or_number, user_id) VALUES ('INCOME', ?, ?, ?, ?, ?)");
                    $stmt->bind_param("dssss", $amt, $date, $desc, $or_num, $admin_id);
                }
...
if(isset($_POST['send_reminder'])){
    ...
    $reminder_type = $_POST['reminder_type']; // 'Down Payment' or 'Monthly Amortization'
    ...
    if($resData && $amount_due > 0){
        ...
        try {
            ...
            $mail->isHTML(true);
            $mail->Subject = "ACTION REQUIRED: $reminder_type Reminder - JEJ Surveying";
            ...
            <div style='font-family: Arial, sans-serif; color: #334155; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.05);'>
                ...
                <div style='padding: 30px; background-color: #ffffff;'>
                    ...
                    <p style='font-size: 15px; line-height: 1.5;'>This is an official payment reminder for your property at <b>Block {$resData['block_no']} Lot {$resData['lot_no']}</b>.</p>
                    <p style='font-size: 15px; line-height: 1.5;'>Please settle your <b>$reminder_type</b> based on the schedule below:</p>
...
if($res && $res->num_rows > 0) {
    while($row = $res->fetch_assoc()){
        ...
        $amort_query = $conn->prepare("SELECT SUM(amount) as total_paid FROM transactions WHERE type='INCOME' AND description LIKE ?");
        ...
        // Calculate Terms
        $terms_years = isset($row['terms']) ? (int)$row['terms'] : 2;
        if($terms_years == 0) $terms_years = 1;
        $balance_to_amortize = $row['total_price'] - $dp_total_required;
        $row['monthly_payment'] = $balance_to_amortize / ($terms_years * 12);
...
<head>
    ...
    <style>
        :root {
            var(--primary): #2e7d32; var(--primary-light): #e8f5e9; var(--dark): #1b5e20; 
            var(--gray-light): #f1f8e9; var(--gray-border): #c8e6c9; var(--text-muted): #607d8b; 
        }
    ...
    <div class="sidebar">
        <div class="brand-box">
            ...
            <div style="line-height: 1.1;">
                <span style="font-size: 16px; font-weight: 800; color: #2e7d32; display: block;">JEJ Surveying</span>
    ...
    <div class="main-panel">
                        ...
                        <?php if(!empty($approved_reservations)): ?>
                            ...
                            <tr>
                                <td>
                                    ...
                                    <span style="font-size: 12px; color: #64748b;"><i class="fa-regular fa-envelope"></i> <?= $row['email'] ?? $row['user_email'] ?></span>
                                </td>
                                <td>
                                    <strong style="color: #2e7d32; font-size: 13px;">Block <?= $row['block_no'] ?> Lot <?= $row['lot_no'] ?></strong>
                                </td>
                                <td>
                                    ...
                                    <?php else: ?>
                                        <div style="font-size: 11px; color: #64748b; margin-top: 6px; padding-top: 6px; border-top: 1px dashed #cbd5e1;">
                                            Amortization Paid: <strong style="color: #3b82f6;">₱<?= number_format($row['amort_paid_amount'], 2) ?></strong><br>
                                            Remaining TCP Balance: ₱<?= number_format($balance - $row['amort_paid_amount'], 2) ?>
                                        </div>
                                    <?php endif; ?>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div style="display: flex; flex-direction: column; gap: 8px;">
                                        ...
                                        <button type="button" class="btn-action btn-record" data-id="<?= (int)$row['id'] ?>" data-balance="<?= $row['dp_remaining'] ?>" data-name="<?= htmlspecialchars($row['fullname']) ?>" data-paid="<?= $row['is_dp_fully_paid'] ? '1' : '0' ?>" onclick="openPaymentModal(this)">
            ...
            <div class="modal-content">
                ...
                <form method="POST" style="padding: 25px;">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="remarks" class="form-control" placeholder="E.g., Paid via GCash...">
                    </div>
                    
                    <div style="margin-top: 25px; text-align: right; border-top: 1px solid #c8e6c9; padding-top: 15px;">
                        <button type="button" onclick="closePaymentModal()" style="background:#f1f5f9; border: 1px solid #cbd5e1; padding: 10px 20px; border-radius: 8px; cursor: pointer; margin-right: 10px;">Cancel</button>
        ...
        function openPaymentModal(btn) {
            ...
            if(btn.dataset.paid == '1') {
                document.getElementById('modal_pay_type').value = 'AMORTIZATION';
                document.getElementById('modal_amount').value = '';
            } else {
                document.getElementById('modal_pay_type').value = 'DP';
